...
class RepairOrderTest < MiniTest::Test
  ...
  def test_that_list_returns_data
    ...
    RepairOrder.stub :list, list_response do
      ...
      assert !(response[:data][0][:relationships][:appointment]).nil?
      assert !(response[:data][0][:relationships][:customer]).nil?
      assert !(response[:data][0][:relationships][:repair_shop]).nil?
      assert !(response[:data][0][:relationships][:vehicle]).nil?
    end
  end
  ...
  def test_that_fetch_returns_data
      ...
      "included": [
        ...
        },
        {
          ...
          "attributes": {
            ...
            "vcdb_base_vehicle_id": 154148,
            "vcdb_engine_config_id": 21030,
    ...
    RepairOrder.stub :fetch, fetch_response do
      ...
      assert !(response[:data][:relationships][:appointment]).nil?
      assert !(response[:data][:relationships][:customer]).nil?
      assert !(response[:data][:relationships][:repair_shop]).nil?
      assert !(response[:data][:relationships][:vehicle]).nil?
    end
  end
end
